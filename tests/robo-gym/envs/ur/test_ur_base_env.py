...
def env(request):
    env = gym.make('EmptyEnvironmentURSim-v0', ip='127.0.0.1', ur_model=request.param, fix_wrist_3=True, gui=True)
...
def test_fixed_joints(env_name, fix_base, fix_shoulder, fix_elbow, fix_wrist_1, fix_wrist_2, fix_wrist_3, ur_model):
    env = gym.make(env_name, ip='127.0.0.1', fix_base=fix_base, fix_shoulder=fix_shoulder, fix_elbow=fix_elbow,
                                                fix_wrist_1=fix_wrist_1, fix_wrist_2=fix_wrist_2, fix_wrist_3=fix_wrist_3, ur_model=ur_model, gui=True)
    state = env.reset()
